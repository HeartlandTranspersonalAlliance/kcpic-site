...
for (const route of [...routes, '404.html']) {
  ...
  assert(
    html.includes(`href="${origin}${base}/${route}"`),
    `canonical ${route}`,
  );
  for (const match of html.matchAll(/(?:href|src)="([^"#]+)"/g)) {
    ...
    if (fragment) {
      const target = readFileSync(
        disk.endsWith('/') ? disk + 'index.html' : disk,
        'utf8',
      );
...
assert(
  meetings.includes('https://www.facebook.com/groups/kcpsychedelic'),
  'Missing meeting destination',
);
assert(
  readFileSync('dist/contact/index.html', 'utf8').includes(
    'mailto:info@kcpic.org',
  ),
  'Missing email',
);
...
const normalized = (s) =>
  decode(s.replace(/<[^>]+>/g, ' '))
    .replace(/\s+/g, ' ')
    .trim();
for (const text of Object.values(content.home))
  assert(
    normalized(home).includes(normalized(text)),
    `Missing home text: ${text.slice(0, 50)}`,
  );
assert(
  normalized(about).includes(normalized(content.about)),
  'About text differs',
);
for (const a of content.agreements)
  assert(
    normalized(about).includes(normalized(a.text)),
    `Missing agreement ${a.title}`,
  );
...
for (const r of content.resources)
  assert(
    resources.includes(r.url) && normalized(resources).includes(r.name),
    `Missing resource ${r.name}`,
  );
assert(existsSync('dist/social.jpg'));
assert(
  readFileSync('dist/sitemap-0.xml', 'utf8').includes(
    origin + base + '/about/',
  ),
);
console.log(
  `Verified 5 pages, 404, base-aware links/assets, metadata, original prose, six agreements and seven resources (${base || '/'}).`,
);
